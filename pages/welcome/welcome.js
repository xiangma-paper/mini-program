...
Page({
  ...
  onLoad: function() {
    ...
    wx.request({
      url: app.host + '/api/wx',
      success: res => this.onWxInit(undefined, res),
      fail: error => this.onWxInit(error)
    })
  },
  onWxInit: function(error, res) {
    if (error) {
      console.log('onWxInit() failed', error)
      wx.hideLoading()
      ...
      api.wxLogin(this.onWxLogin)
    }
  },
  onWxLogin: function(error, res) {
    if (error) {
      console.error('onWxLogin() failed:', error)
      wx.hideLoading()
  ...
  onEnter: function() {
    if (this.validateNickname()) {
      ...
      api.updateNickname(this.data.nickname, this.onUpdateNickname)
    }
  },
  onUpdateNickname: function(error, res) {
    wx.hideLoading()
    if (error) {
  ...
  gotoNextPage: function() {
    console.log('gotoNextPage() before')
    wx.switchTab({url: '/pages/checkin/checkin'})
    console.log('gotoNextPage() after')
  }
})